...
// [新增] 等待高德地图脚本就绪
// 作用：解决“DOMContentLoaded 已触发，但 AMap 仍未准备好”时直接报错的问题
async function ensureAMapReady(timeout = 8000, interval = 200) {
...
async function initData() {
	try {
		...
		// [新增] 如果地图还在加载层状态，则给出更明确提示
		if (MapController.map) {
			// 地图存在时，仅记录错误，不覆盖地图
		} else {
			MapController.showMapError("地图或项目数据加载失败，请稍后重试");
		}
	}
}
...
function fillForm(p) {
	...
	if (mediaArea) {
		...
		if (p.media_files && mediaList) {
			// 生成带操作按钮的缩略图结构
			mediaList.innerHTML = p.media_files
				.map(
					(m, index) => `
...
function renderDetailModal(p, mode = "default") {
	...
	if (p.media_files && p.media_files.length > 0) {
		const indicators = p.media_files
			.map(
				(_, i) =>
		...
		carouselHtml =
			'<div class="text-center p-4 text-muted bg-light">暂无现场照片</div>';
	}
...
document.addEventListener("DOMContentLoaded", async () => {
	// [关键调整]
	// 先完成“不会依赖地图成功与否”的初始化，避免地图失败拖死整个页面
	app.initRouter();
	app.initTheme();
	...
	// 地图相关初始化放到受控流程里
	try {
		MapController.showMapLoading("正在等待高德地图脚本...");
		await ensureAMapReady(8000, 200);

		MapController.initMap("amap-container");
		MapController.initPicker("picker-map", "input-lat", "input-lng");
	} catch (err) {
		console.error("地图初始化失败:", err);
		MapController.showMapError("地图初始化失败，请稍后刷新重试");
	}

	// 数据初始化单独执行：即使地图失败，也尽量让其余模块继续可用
	try {
		await initData();
	} catch (err) {
		console.error("首屏数据初始化失败:", err);
	}
});
